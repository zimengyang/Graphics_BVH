...
Material* XMLReader::LoadMaterial(QXmlStreamReader &xml_reader, const QStringRef &local_path)
{
    ...
    else if(QStringRef::compare(type, QString("phong")) == 0)
    {
        ...
        QStringRef spec_col = attribs.value(QString(), QString("specularColor"));
        if(QStringRef::compare(spec_col, QString("")) !=0)
        {
            ((PhongMaterial*)result)->specular_color = ToVec3(spec_col);
        }
    }
    else
    {
